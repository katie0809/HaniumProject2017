...
def getMapMetadata(mapDataServiceName):
    # Create service handle
    mapDataService = rospy.ServiceProxy(mapDataServiceName, GetMap)
    mapMetaDataService = mapDataService().map.info
...
def exploreNode():
    ...
    rospy.loginfo("Waiting for service " + mapDataServiceName + ".")
    rospy.wait_for_service(mapDataServiceName)
    rospy.loginfo("Found service " + mapDataServiceName + ".")

    mapMetaData = getMapMetadata(mapDataServiceName)
    if mapMetaData == None:
...
#    mapMetaDataServiceName = rospy.get_param('~mapMetaDataServiceName', '/dynamic_map_metadata')
    mapDataServiceName = rospy.get_param('~mapDataServiceName', '/dynamic_map')
